...
public abstract class Subscriber extends RunnableSubPub {
    ...
    private ConcurrentHashMap<String,Callback> broadcastToBeCalled;
    private ConcurrentHashMap<String,Callback> EventToBeCalled;
    ...
    public Subscriber(String name) {
        super(name);
        this.broadcastToBeCalled = new ConcurrentHashMap<String, Callback>();
        this.EventToBeCalled = new ConcurrentHashMap<String, Callback>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        EventToBeCalled.put(type.getName(),callback);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        broadcastToBeCalled.put(type.getName(),callback);
    ...
    public final void run() {
        ...
        while (!terminated){
            try {
                msg = messageBroker.awaitMessage(this);
                if (msg instanceof Event)
                    call_ = EventToBeCalled.get(msg.getClass().getName());

                else if(msg instanceof Broadcast)
                    call_ = broadcastToBeCalled.get(msg.getClass().getName());
                call_.call(msg);
            }
